...
public class MainActivity extends AppCompatActivity {
    ...
    private Button[] buttonsCal = new Button[idCal.length];     //定义运算符按钮数组
    private Button[] buttonsNum = new Button[idNum.length];     //定义数字按钮数组
    private Button buttonEqu;   //=
    private Button buttonClear;  // AC
    private Button buttonDel;    // 删除
    private EditText input ;     //输入框
    private TextView output;    //显示结果文本框
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        input.setText("");
        input.setEnabled(false); //该控件将不再响应点击、触摸以及键盘事件等，处于完全被禁用的状态
        ...
        buttonEqu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                output.setText(new Calculate(name).str);
            }
        });
    ...
    class CalOnClick implements  View.OnClickListener{
        ...
        public void onClick(View v) {
            if (!output.getText().toString().equals("")) {
                ...
                if(output.getText().toString().equals("ERROR")||output.getText().toString().equals("Infinity"))
                    input.setText("");
                else
                ...
                if (Msg.equals(calSymbol[i])) {
                    for (int j = 0; j < calSymbol.length; j++) {
                        if (input.getText().toString().split("")
                                [input.getText().toString().split("").length - 1].equals(calSymbol[j])) {
                            if (!input.getText().toString().isEmpty()) {
                                Text = input.getText().toString();
                                Text = Text.substring(0, Text.length() - 1);
                                input.setText(Text);
...
//            }
    ...
    public class Calculate {
         ...
         */
        public List<String> midToAfter(List<String> midList) throws Exception {
